...
import { bookingsStore } from "@/lib/dummy-data";

interface Props {
  roomId?: string; // if provided, only count bookings for that room
  selectedDate?: string;
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 sm:p-6">
      ...
      <div className="mt-5 flex items-center gap-5 text-xs text-muted-foreground">
        <div className="flex items-center gap-2">
          <span className="h-2.5 w-2.5 rounded-full bg-primary" /> Ada Booking
        </div>
        <div className="flex items-center gap-2">
          <span className="h-2.5 w-2.5 rounded-full bg-muted" /> Kosong
        </div>
      </div>
    </div>
  );
